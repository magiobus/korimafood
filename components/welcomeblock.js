...
const sheetsUrls = {
  chihuahua: { name: process.env.NEXT_PUBLIC_CUU_SHEET_NAME, sheetIndex: 0 },
  juarez: { name: process.env.NEXT_PUBLIC_JUAREZ_SHEET_NAME, sheetIndex: 1 },
  elpaso: { name: process.env.NEXT_PUBLIC_ELP_SHEET_NAME, sheetIndex: 2 },
  cuauhtemoc: {
    name: process.env.NEXT_PUBLIC_CUAUHTEMOC_SHEET_NAME,
    sheetIndex: 3,
  },
  parral: { name: process.env.NEXT_PUBLIC_PARRAL_SHEET_NAME, sheetIndex: 4 },
  delicias: {
    name: process.env.NEXT_PUBLIC_DELICIAS_SHEET_NAME,
    sheetIndex: 5,
  },
  meoqui: { name: process.env.NEXT_PUBLIC_MEOQUI_SHEET_NAME, sheetIndex: 6 },
  creel: { name: process.env.NEXT_PUBLIC_CREEL_SHEET_NAME, sheetIndex: 7 },
};
...
const WelcomeBlock = (props) => {
  ...
  const handleClick = async () => {
    ...
    try {
      let response = await axios.post("/api/getplace", {
        name: sheetsUrls[searchPlace].name,
      });
      ...
      ShowErrorToast(
        toast,
        "Ocurrió un error",
        "No pudimos traer tu recomendación u.u"
      );
    }
  };
  ...
  return (
    <>
      ...
      {!showPlace && !isLoading && (
        <Flex align="center" justify="center" direction="column" mx={2}>
          <Box
            ...
            width="100%"
          >
            <Heading as="h1" size={["2xl"]} color="red.500" mb={3}>
              {heading}
            </Heading>
            <Text
              textAlign="center"
            ...
            >
              <b>
                {description}
                <Text color="red.300" width="100%" mt={2} px={4} py={2}>
                  {place}
                </Text>
              </b>
            </Text>
      ...
      {isLoading && (
        <Loading message="Recorriendo la ciudad en busqueda de tu nuevo lugar favorito" />
      )}

      {/* ---------- Showing Place Component -------- */}
      {showPlace && !isLoading && places.length > 0 && (
        <Place
          places={places}
          sheetIndex={sheetsUrls[searchPlace].sheetIndex}
        />
      )}
    </>
  );
